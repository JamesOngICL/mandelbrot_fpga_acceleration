...
int main(int argc, char** argv) {
    ...
    cl::Kernel krnl_mandelbrot_compute;
    ...
    int8_t in_data_0[30000];
    for (unsigned i = 0; i < 30000; i++) {
      ifile0 >> in_data_0[i];
    }
    size_t size_bytes_in0 = sizeof(int8_t) * 30000;
    std::vector<int8_t, aligned_allocator<int8_t> > source_in0(in_data_0, in_data_0 + 30000);
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_mandelbrot_compute = cl::Kernel(program, "mandelbrot_compute", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, cl::Buffer buffer_in0(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, size_bytes_in0, source_in0.data(), &err));

    OCL_CHECK(err, err = krnl_mandelbrot_compute.setArg(0, buffer_in0));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(krnl_mandelbrot_compute, nullptr, &event));

    // Copy Result from Device Global Memory to Host Local Memory
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_in0}, CL_MIGRATE_MEM_OBJECT_HOST));
    q.finish();
    ...
    std::cout << "| " << std::left << std::setw(24) << "mandelbrot_compute "
    ...
    for (unsigned i = 0; i < source_in0.size(); i++) {
        ofile << source_in0[i] << std::endl;
    }
    ofile.close();
